...
- 비활성 버튼 클릭 시 한국어·대상 언어 안내 토스트 표시
- 비활성화된 버튼은 `<a>` 대신 `<button type="button">`으로 렌더링하여 크롤러가 링크를 수집하지 않음
- 언어 선택 컴포넌트 전체를 `data-nosnippet` 컨테이너로 감싸 현재 언어명과 지원 언어 목록이 검색 결과 설명에 사용되지 않게 함
