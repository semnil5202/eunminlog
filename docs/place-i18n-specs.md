...
| `place.address` | 주소 라벨 |
| `place.price` | 가격대 라벨 |
| `place.copyOriginal` | 토스트 메시지 (원문 복사 안내) |
...
<div class="flex items-baseline gap-3">
  <dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.name", locale)}</dt>
  <dd class="flex items-baseline gap-1.5">
    ...
    <button
      ...
      data-copy={placeName}
      data-toast={locale !== 'ko' ? t("place.copyOriginal", locale) : undefined}
...
<!-- 주소 행 -->
<div class="flex items-baseline gap-3">
  <dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.address", locale)}</dt>
  <dd class="flex items-baseline gap-1.5">
...
| `place.price` | 가격대 | Price Range | 価格帯 | 价格区间 | 價格區間 | Kisaran Harga | Khoảng giá | ช่วงราคา |
| `place.copyOriginal` | (미사용) | Copied in Korean for accurate search | 正確な検索のため韓国語の原文をコピーしました | 已复制韩语原文以便准确搜索 | 已複製韓語原文以便準確搜尋 | Disalin dalam bahasa Korea untuk pencarian akurat | Đã sao chép bằng tiếng Hàn để tìm kiếm chính xác | คัดลอกเป็นภาษาเกาหลีเพื่อค้นหาได้แม่นยำ |

### 6-2. 한국어 locale에서 `place.copyOriginal` 미사용
...
**변경 후**:
```html
<dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.category", locale)}</dt>
<dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.name", locale)}</dt>
<dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.address", locale)}</dt>
<dt class="shrink-0 w-16 text-gray-500 font-medium">{t("place.price", locale)}</dt>
```

> `<dt>` 너비(`w-16` = 64px)가 일부 언어에서 텍스트를 수용하지 못할 수 있다. `w-16`을 `w-20`(80px)으로 확장하거나, `w-fit min-w-16`으로 가변 처리할 수 있다. SE가 구현 시 각 언어 텍스트 길이를 확인하여 결정한다.
