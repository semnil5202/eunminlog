...
쿠팡 파트너스에서 300×250, 680×140, In-feed 전용 680×280 다이나믹 iframe을 지면별로 생성했다. Article은 화면 폭과 무관하게 홀수 순번 5개에 680×140, 짝수 순번 5개에 300×250을 교차 배치한다. Article 1·4와 Sidebar는 각각 별도 고객 관심 기반 추천 위젯으로 성과를 실험하며 나머지는 카테고리 베스트다. Search와 비활성 Feed는 화면 폭 분기 없이 680×280을 사용하고 Sidebar는 300×250을 유지한다.

Feed fallback 설정과 widget ID는 재활성화를 위해 보존하지만 해당 슬롯 키가 비활성인 동안 호출하지 않는다. Search fallback은 활성 상태다.

| 지면           | 카테고리       | 규격    | ID        | 선택 규칙        |
| -------------- | -------------- | ------- | --------- | ---------------- |
| PostTop        | 반려동물용품   | 728×90  | `1013691` | 고정 배너 축소   |
| Feed index 1   | 주방용품       | 680×280 | `1014211` | 현재 비활성      |
| Feed index 4   | 식품           | 680×280 | `1014212` | 현재 비활성      |
| Article 1      | 고객 관심 기반 | 680×140 | `1014243` | 홀수·추천 실험   |
| Article 2      | 헬스·건강식품  | 300×250 | `1013229` | 짝수 순번        |
| Article 3      | 식품           | 680×140 | `1014085` | 홀수 순번        |
| Article 4      | 고객 관심 기반 | 300×250 | `1015105` | 짝수·추천 실험   |
| Article 5      | 주방용품       | 680×140 | `1014089` | 홀수 순번        |
| Article 6      | 생활용품       | 300×250 | `1013219` | 짝수 순번        |
| Article 7      | 뷰티           | 680×140 | `1014087` | 홀수 순번        |
| Article 8      | 홈인테리어     | 300×250 | `1013428` | 짝수 순번        |
| Article 9      | 출산·유아동    | 680×140 | `1014091` | 홀수 순번        |
| Article 10     | 반려동물용품   | 300×250 | `1013426` | 짝수 순번        |
...
- 고정 배너는 원본 비율과 명시 크기를 유지한다. Native AdSense는 기존 `fluid` 형식을 유지하며 In-feed는 `min-height: 280px`, In-article은 `min-height: 250px`를 예약한다. 쿠팡 fallback 크기는 AdSense 요청 크기를 변경하지 않는다. 동적 위젯은 `overflow-hidden`으로 상품, CTA, 광고 표기를 자르지 않으며 field p75 CLS 0.1 이하를 검증한다.
...
- 고객 관심 기반 추천은 Article 1·4와 Sidebar에서 각각 별도 ID로 카테고리 베스트와 성과를 비교한다. 반복 구매 가능성이 높은 헬스·식품·주방·생활·뷰티를 Article 2·3·5~7에 우선 배치하고, Article 8~10은 홈인테리어·출산/유아동·반려동물로 다양성을 확보한다. 배포 후 코드에서 참조하지 않는 기존 다이나믹 위젯 13개는 쿠팡 파트너스에서 삭제했다.
...
- Sidebar의 고객 관심 기반 추천은 PC에서만 노출되므로 Article 1의 모바일 포함 성과와 직접 합산하지 않는다. 기기·지면별 슬롯 조회 대비 쿠팡 클릭·주문·수익을 비교하며, 쿠팡 리포트가 widget ID 구분을 제공하지 않으면 생성 코드에서 지원하는 별도 추적 식별자를 사용한다.
