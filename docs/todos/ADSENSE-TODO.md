...
쿠팡 파트너스에서 Mobile·중간 폭용 300×250과 PC 본문용 680×140 다이나믹 iframe을 지면별로 생성했다. Article·Search와 비활성 Feed 설정은 `xl`(1280px) 미만에서 300×250, 이상에서 680×140을 사용하고 Sidebar는 300×250을 유지한다. 동적 상품은 위치별로 다른 카테고리를 사용하지만 실제 노출 상품은 쿠팡 응답에 따라 달라진다.

Feed fallback 설정과 widget ID는 재활성화를 위해 보존하지만 해당 슬롯 키가 비활성인 동안 호출하지 않는다. Search fallback은 활성 상태다.

| 지면           | 카테고리      | `< xl` 300×250 ID | `xl` 680×140 ID | 비고              |
| -------------- | ------------- | ----------------- | --------------- | ----------------- |
| PostTop        | 반려동물용품  | `1013691`         | --              | 고정 728×90 축소  |
| Feed index 1   | 주방용품      | `1013218`         | `1014089`       | 현재 비활성       |
| Feed index 4   | 식품          | `1013216`         | `1014085`       | 현재 비활성       |
| Article 1      | 식품          | `1013216`         | `1014085`       | 활성              |
| Article 2      | 생활용품      | `1013219`         | `1014086`       | 활성              |
| Article 3      | 뷰티          | `1013228`         | `1014087`       | 활성              |
| Article 4      | 헬스·건강식품 | `1013229`         | `1014088`       | 활성              |
| Article 5      | 주방용품      | `1013218`         | `1014089`       | 활성              |
| Article 6      | 반려동물용품  | `1013426`         | `1014090`       | 활성              |
| Article 7      | 출산·유아동   | `1013427`         | `1014091`       | 활성              |
| Article 8      | 홈인테리어    | `1013428`         | `1014092`       | 활성              |
| Article 9      | 스포츠·레저   | `1013429`         | `1014093`       | 활성              |
| Article 10     | 가전디지털    | `1013220`         | `1014094`       | 활성              |
| Search index 1 | 식품          | `1013216`         | `1014085`       | 활성              |
| Search index 4 | 뷰티          | `1013228`         | `1014087`       | 활성              |
| Sidebar        | 헬스·건강식품 | `1013229`         | --              | PC 300×250만 사용 |
...
- 쿠팡 전환 시점에 `(min-width: 1280px)`을 한 번 평가해 해당 크기의 iframe `src` 하나만 설정한다. 이후 resize에서는 위젯을 교체하지 않아 중복 요청·중복 집계를 방지한다.
...
- 고정 배너는 원본 비율과 명시 크기를 유지한다. Native AdSense는 기존 `fluid` 형식과 `min-height: 250px` 예약 영역을 그대로 사용하므로 쿠팡 fallback의 680×140 크기가 AdSense 요청 크기를 변경하지 않는다. 동적 위젯은 `overflow-hidden`으로 상품, CTA, 광고 표기를 자르지 않으며 field p75 CLS 0.1 이하를 검증한다.
