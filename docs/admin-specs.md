...
> Status: Phase 1 완료, Phase 3 Tiptap 에디터 + 이미지 삽입 + 폼 형식/메타 폼/번역/폼 검증/UX 부분 구현 완료 (저장/S3 업로드 미구현), Phase 2 진행 예정
...
| PE-17 | 폼 검증 — 필수 입력값 미입력 시 "작성 완료" 버튼 비활성화 (런타임 검증)                | P0       | 구현 완료 |
...
"작성 완료" 버튼은 아래 조건이 모두 충족될 때만 활성화된다. Zod schema 기반 검증은 향후 구현 예정.

| 조건 | 대상 필드 | 폼 형식 |
| ---- | --------- | ------- |
| 제목 입력 | `title.trim()` | 공통 |
| 본문 입력 | `content.trim()` | 공통 |
| 썸네일 업로드 | `thumbnail` | 공통 |
| 카테고리 선택 | `category` | 공통 |
| 서브카테고리 선택 | `subCategory` | 공통 |
| 3줄 요약 입력 | `description.trim()` | 공통 |
| 장소명 입력 | `placeName.trim()` | visit 전용 |
| 주소 입력 | `address.trim()` | visit 전용 |
| 가격 입력 | `price.trim()` | visit 전용 |
| 번역 완료 | `isTranslated` | 카테고리 선택 시 필수 |

추가 비활성화 조건:
- "번역본 생성하기" 버튼: 3줄 요약(`description`)이 비어있으면 비활성화
...
| 버튼 | 표시 조건 | 비활성화 조건 |
| ---- | --------- | ------------- |
| 번역본 생성하기 | `needsTranslation && !isTranslated && flaggedTerms.length === 0` | `isExtracting \|\| !description.trim()` |
