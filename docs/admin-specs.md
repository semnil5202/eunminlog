...
> Status: Phase 1 완료, Phase 3 Tiptap 에디터 + 이미지 삽입 + 폼 형식/메타 폼/번역 부분 구현 완료 (저장/S3 업로드 미구현), Phase 2 진행 예정
...
| `translation`     | GPT-4o 다국어 번역                 | 에디터 내 사용                   | 부분 구현 (고유명사 추출 + 번역 + 미리보기 Sheet) |
...
- 액션 버튼: "번역본 생성" (카테고리 선택 + 미번역 시) / "번역본 확인하기" (번역 완료 시) / "작성 완료"
...
> **현재 구현 상태**: 고유명사 추출 + 번역 실행 + 미리보기 Sheet까지 구현 완료. 번역 결과 DB 저장(post_translations UPSERT)은 미구현. 에디터 페이지(`/posts/new`)에서 직접 연동하여 사용.
...
| 7     | translation (완성)   | GPT-4o 번역 저장, 번역 상태 관리                                                                     | P0       | 미착수                                                               |
